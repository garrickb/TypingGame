...
Monster.prototype.update = function () {
    ...
    var b = this.destY - this.y;
    var c = this.attributes[AttributeEnum.MOVE_SPEED];
    if (b != 0) {
        if (a > 0) {
            this.x += ((a > c / 2 * speedMod) ? c / 2 * speedMod : a)
        } else {
            if (a < 0) {
                this.x -= ((a < c / 2 * speedMod) ? c / 2 * speedMod : -a)
            } else {
...
Monster.prototype.draw = function () {
    if (this.currentHP > 0) {
        try {
            var b = loadedMonsters[this.attributes[AttributeEnum.SRC]];
            var c = b.width * imgScale;
            var a = b.height * imgScale;
            ctx.drawImage(b, this.x, this.y, c, a)
        } catch (d) {
            console.log(this.attributes)
        }
        ctx.beginPath();
        ctx.fillStyle = "green";
        ctx.rect(this.x, this.y - 5, (this.currentHP / this.attributes[AttributeEnum.HP]) * c, 15);
        ctx.fill();
        ctx.lineWidth = 2;
        ...
        ctx.beginPath();
        ctx.fillStyle = "red";
        ctx.rect(this.x + (this.currentHP / this.attributes[AttributeEnum.HP]) * c, this.y - 5, c - ((this.currentHP / this.attributes[AttributeEnum.HP]) * c), 15);
        ctx.fill();
        ctx.lineWidth = 2;
        ctx.strokeStyle = "black";
        ctx.stroke()
...
function getWord(a) {
    ...
    switch (a) {
        case 0:
        case 1:
            f = ["mom", "dad", "eye", "leg", "arm", "pig", "hen", "cat", "dog", "god", "well", "tree", "yes", "no", "sun", "food", "snow", "day", "tall", "short", "hot", "warm", "poor", "rich", "bull", "ox", "elk", "lion", "bear", "wolf", "goat", "hand", "neck", "nail", "mad", "evil", "here", "over", "back", "there", "why", "how", "now", "time", "moo", "beep", "what", "lap", "coin", "cup", "box", "show", "more", "less", "kill", "kills", "bar", "bars", "barn", "loud", "lord", "sword", "rat", "bag", "war", "axe", "hello", "apple", "pear", "liar", "the", "zen"];
            break;
        case 2:
            f = ["bottle", "spooky", "snatch", "squat", "control", "satchel", "sanitize", "keyboard", "lights", "poster", "sword", "battle", "hatchet", "helmet", "plate", "nobody", "shirt", "underwear", "hello there", "scrape", "the cat", "a bat", "an apple", "banana", "lying", "squire", "knight", "boulder", "rocking", "living", "hitting", "dying", "paper cut", "wounded", "zen"];
            break;
        default:
            return"ERROR";
            break
...
function render() {
    ...
    if (activeScreen == ScreenEnum.MainMenu) {
        ...
        if (activeScreen == ScreenEnum.Game || activeScreen == ScreenEnum.Inventory || activeScreen == ScreenEnum.Store) {
            ...
            ctx.moveTo(c + a, g);
            ctx.lineTo(c + f - a, g);
            ctx.quadraticCurveTo(c + f, g, c + f, g + a);
            ctx.lineTo(c + f, g + b - a);
            ctx.quadraticCurveTo(c + f, g + b, c + f - a, g + b);
            ctx.lineTo(c + a, g + b);
            ctx.quadraticCurveTo(c, g + b, c, g + b - a);
            ctx.lineTo(c, g + a);
            ctx.quadraticCurveTo(c, g, c + a, g);
            ctx.closePath();
            ctx.stroke();
            ctx.fill();
            ctx.fillStyle = "black";
            ctx.fillText(currentWord, c + 4, g + 16);
            if (focus) {
                ctxInfo.fillStyle = "white";
                ctxInfo.fillRect(0, 0, infoCanvas.width, infoCanvas.height);
